...
// Order is preference order: the first available entry for a codec wins.
//
// Quick Sync sits ahead of VAAPI deliberately. On Intel both are detected, but the VAAPI
// path uses fixed-quantiser CQP while the QSV path uses ICQ — a real constant-quality mode,
// and the closest hardware analog to CRF. Picking VAAPI on an Intel GPU was choosing the
// worse of two available paths.
